...
#				print("hash={} nonce={} value={} target={}".format(hash_result, nonce, int(hash_result, 16), target))
    ...
    def valid(self):
        if self.hash == '':  # a hash should never be blank
            ...
            # if the temp block resultant hash is not that of self.hash return err
            if (tmp.hash != self.hash):
        ...
        # check if diff bits is over or equal to difficulty for height self.height
        elif (self.diff_bits < DIFFICULTYS[block.algo]):
