...
for document in cursor:
  ...
  if (txn.valid()):
    ...
    if (mempool.in_mempool(txn.hash) == False):
      ...
      log.trace("Ignoring txn, already in mempool. hash={}".format(txn.hash))
  else:
    log.trace("Loaded invalid unconfirmed txn, discarding. hash={}".foramt(txn.hash))
...
def set(key, value, collection_name):
  ...
  if (collection.find_one({"_id": "{}".format(key)}) != None):
    ...
    collection.insert_one( { "_id" : "{}".format(key), "data" : "{}".format(value) } )
    return True
